...
# 수정 제출 : 275ms / 메모리 38MB 매 턴마다 플레이어들을 리스트가 아닌 딕셔너리로 저장
    ...
    for i in range(N):
        for j in range(N):
            ...
            if len(players_turns[i][j]) != 0:
                ...
                '''
                기존의 players들은 리스트여서 for문을 돌았지만, 딕셔너리로 players들을 수정하여
                단순하게 key값으로 존재하는 플레이어 판단 및 좌표값으로 시간 줄임
                '''
                player = players_turns[i][j][0]
                if player in players and players[player][0] == i and players[player][1] == j:
                    continue
...
for i in range(N):
    for j in range(N):
        ...
        if board[i][j] != 0:
            player = board[i][j]
            players[player] = [i, j, first_directions[player - 1]]
